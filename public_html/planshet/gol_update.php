...
$recordS = $db->Execute("select turnir, team1, team2 from v9ky_match where id='".$matcid."'");



class Validation {
    
    public $text1 = "<tr><td>ID</td><td>Команда</td><td>Гравець</td><td>Хвилина</td><td><img width=30px height=30px src='https://v9ky.in.ua/planshet/picts/ball.jpg'></td></tr>";
    ...
    public function sel_all(){
        ...
        $test1 = $this -> text1;
        $recordS1 = $db->Execute("select * from v9ky_gol where matc='".$matcid."' ORDER BY time");
        while (!$recordS1->EOF) {
	  ...
	  $recordS3 = $db->Execute("select * from v9ky_player where id='".$recordS1->fields[player]."'"); 
	    ...
	    <td>".$recordS2->fields[name]."</td><td>";
          if ($recordS3->fields[player]=="999") $test1 .= "Техпоражение";
	  else $test1 .= $recordname->fields[name1]." ".$recordname->fields[name2];
          $test1 .= "</td>
    ...
    public function create_man($team, $time, $player){
      ...
      //запись в базу
      $db->AutoExecute('v9ky_gol',$record,'INSERT');
	  
	$recordGols1 = $db->Execute("select count(*) as kol from v9ky_gol where team=".$recordS->fields[team1]." and matc=".$matcid."");
	$recordGols2 = $db->Execute("select count(*) as kol from v9ky_gol where team=".$recordS->fields[team2]." and matc=".$matcid."");
	$record_gol_to_matc["gols1"] = $recordGols1->fields[kol];
	$record_gol_to_matc["gols2"] = $recordGols2->fields[kol];
	$record_gol_to_matc["upd_teams_match_stat"] = 1;
	$db->AutoExecute('v9ky_match',$record_gol_to_matc,'UPDATE', 'id = '.$matcid.'');

      $recordt["upd_table"] = 1;
      $db->AutoExecute('v9ky_turnir',$recordt,'UPDATE', 'id = '.$recordS->fields[turnir].'');
    ...
    public function del_man($idnum){
      ...
      global $recordS;
	  global $matcid;
      $db->Execute("delete from v9ky_gol where id='".$idnum."'");
    ...
    public function activer($active){
      ...
      //$record["updatet"] = gmdate('Y-m-d H:i:s');
      //$db->AutoExecute('v9ky_turnir',$record,'UPDATE', 'id = '.$recordS->fields[turnir].'');
      $test = $active;
      return $test;
    }
}
...
//$recordGols1 = $db->Execute("select count(*) as kol from v9ky_gol where team=".$recordS->fields[3]." and matc=".$matcid."");
//$recordGols2 = $db->Execute("select count(*) as kol from v9ky_gol where team=".$recordS->fields[4]." and matc=".$matcid."");
?>  
<center>
...
<td><input type='hidden' name='matcid' value='".$matcid."'><button type='submit' onclick='goinfo();'><img width=40px height=40px src='https://v9ky.in.ua/img/anons.png' 
          style='vertical-align: middle'> ІНФО</button></td>
<td><input type='hidden' name='matcid' value='".$matcid."'><button type='submit' onclick='goyellow();'><img width=30px height=40px src='https://v9ky.in.ua/planshet/picts/yel.jpg' 
          style='vertical-align: middle'> ЖОВТІ</button></td>
...
<td><input type='hidden' name='matcid' value='".$matcid."'><button type='submit' onclick='gored();'><img width=30px height=40px src='https://v9ky.in.ua/planshet/picts/red.jpg' 
...
<?
  echo "<table><tr><td>В пользу команды</td><td>Игрок</td><td>Минута</td><td></td></tr>
	 ...
	 echo"<option value='999'>Техпоражение</option> \n</select>";
	 
	 echo" </td>
	 <td><input type='number' min='0' id='time1' name='time1' size='3' ></td><td>";
  echo "<input type='submit' value='  Создать гол  ' onclick='validate1();'></td></tr></table>  ";
...
echo "<table>
  ...
  echo "<input type='submit' value='  Создать гол  ' onclick='validate2();'></td></tr></table><br>  ";



$recordS1 = $db->Execute("select * from v9ky_gol where matc='".$matcid."' ORDER BY time desc");
