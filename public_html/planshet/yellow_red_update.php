...
class Validation {
    ...
    public $text1 = "<tr><td>ID</td><td>Команда</td><td>Гравець</td><td>Хвилина</td><td><img width=20px height=27px src='https://v9ky.in.ua/planshet/picts/yellow-red-icon.png'></td></tr>";
    ...
    public function create_man($team, $time, $player){
      ...
      //запись в базу
      $db->AutoExecute('v9ky_yellow_red',$record,'INSERT');
    ...
    public function del_man($idnum){
      ...
      global $recordS;
	  global $matcid;
      $db->Execute("delete from v9ky_yellow_red where id='".$idnum."'");
      $record_gol_to_matc["upd_teams_match_stat"] = 1;
	  $db->AutoExecute('v9ky_match',$record_gol_to_matc,'UPDATE', 'id = '.$matcid.'');
      $test = $this -> sel_all();
...
<?  
  ...
  echo "<input type='submit' value='  Створити Ж-КК ' onclick='validate1();'></td></tr></table>";
  ...
  echo "<input type='submit' value='  Створити Ж-КК ' onclick='validate2();'></td></tr></table>";
