...
class Validation {
    ...
    public $text1 = "<tr><td>ID</td><td>Команда</td><td>Гравець</td><td>Хвилина</td><td><img width=30px height=30px src='https://v9ky.in.ua/planshet/picts/asist.jpg'></td></tr>";
    ...
    public function sel_all(){
        ...
        $test1 = $this -> text1;
        $recordS1 = $db->Execute("select * from v9ky_asist where matc='".$matcid."' ORDER BY time");
        while (!$recordS1->EOF) {
    ...
    public function create_man($team, $time, $player){
      ...
      //запись в базу
      $db->AutoExecute('v9ky_asist',$record,'INSERT');
    ...
    public function del_man($idnum){
      ...
      global $recordS;
	  global $matcid;
      $db->Execute("delete from v9ky_asist where id='".$idnum."'");
      $record_gol_to_matc["upd_teams_match_stat"] = 1;
	  $db->AutoExecute('v9ky_match',$record_gol_to_matc,'UPDATE', 'id = '.$matcid.'');
      $test = $this -> sel_all();
...
$recordS1 = $db->Execute("select * from v9ky_asist where matc='".$matcid."' ORDER BY time desc");
