...
class Validation {
    ...
    public $text1 = "<tr><td>ID</td><td>Команда</td><td>Гравець</td><td>Хвилина</td><td>Статус (0/1)</td><td><img width=27px height=27px src='https://v9ky.in.ua/planshet/picts/football-penalty-6m.png'></td></tr>";
    ...
    public function sel_all(){
        ...
        $test1 = $this -> text1;
        $recordS1 = $db->Execute("select * from v9ky_penalty where matc='".$matcid."' ORDER BY time");
        while (!$recordS1->EOF) {
    ...
    public function create_man($team, $time, $player, $status){
      global $db;
      global $matcid;
      global $recordS;
      ...
      //запись в базу
      $db->AutoExecute('v9ky_penalty',$record,'INSERT');
    ...
    public function del_man($idnum){
      ...
      global $recordS;
	  global $matcid;
      $db->Execute("delete from v9ky_penalty where id='".$idnum."'");
      $record_gol_to_matc["upd_teams_match_stat"] = 1;
	  $db->AutoExecute('v9ky_match',$record_gol_to_matc,'UPDATE', 'id = '.$matcid.'');
      $test = $this -> sel_all();
...
function validate1(){
    ...
    val4 = document.getElementById("status1").value;
	
    myClass.create_man(val1, val2, val3, val4, {
...
function validate2(){
    ...
    val4 = document.getElementById("status2").value;
...
<td><input type='hidden' name='matcid' value='".$matcid."'><button type='submit' onclick='goyellow();'><img width=40px height=40px src='https://v9ky.in.ua/planshet/picts/yel.jpg' 
          style='vertical-align: middle'> ЖОВТІ</button>
</td>
...
<?  
  echo "<table><tr><td>Команда</td><td>Гравець</td><td>Хвилина</td><td><span title='0 - невдалий пенальті, 1 - вдалий пенальті'>Статус (0/1)*</span></td></tr>
     <tr><td>";
	 ...
	 echo" </td>
	 <td><input type='number' min='0' id='time1' name='time1' size='3' ></td><td><input type='number' min='0' max='1' id='status1' name='status1' size='3' ></td><td>";
  echo "<input type='submit' value='  Створити ПЕНАЛЬТІ ' onclick='validate1();'></td></tr>";
  ...
  echo "<tr><td>";
	 ...
	 echo" </td>
	 <td><input type='number' min='0' id='time2' name='time2' size='3' ></td><td><input type='number' min='0' max='1' id='status2' name='status2' size='3' ></td><td>";
  echo "<input type='submit' value='  Створити ПЕНАЛЬТІ ' onclick='validate2();'></td></tr></table>";
