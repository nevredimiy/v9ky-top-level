...
class Validation {
    ...
    public $text1 = "<tr><td>ID</td><td>Команда</td><td>Гравець</td><td>Хвилина</td><td><img width=20px height=27px src='https://v9ky.in.ua/planshet/picts/red.jpg'></td></tr>";
    ...
    public function sel_all(){
        ...
        $test1 = $this -> text1;
        $recordS1 = $db->Execute("select * from v9ky_red where matc='".$matcid."' ORDER BY time");
        while (!$recordS1->EOF) {
    ...
    public function create_man($team, $time, $player){
      ...
      //запись в базу
      $db->AutoExecute('v9ky_red',$record,'INSERT');
    ...
    public function del_man($idnum){
      ...
      $db->Execute("delete from v9ky_red where id='".$idnum."'");
      $record_gol_to_matc["upd_teams_match_stat"] = 1;
	  $db->AutoExecute('v9ky_match',$record_gol_to_matc,'UPDATE', 'id = '.$matcid.'');
      $test = $this -> sel_all();
...
<td><button type='submit' onclick='goyellow();'><img width=30px height=40px src='https://v9ky.in.ua/planshet/picts/yel.jpg' 
...
<?  
  if ((isset($_GET['id']))&&(intval($_GET['id'])*1>0)&&(($redatirovat_or_else==1)||(!isset($_GET['red'])))) 
  {
	 ...
	 $recordSet1 = $db->Execute("select * from v9ky_red where id='".$id_to_update."'");
  }else {
    $recordSet1 = $db->Execute("select * from v9ky_red where id=(SELECT LAST_INSERT_ID())");
  ...
  echo "<input type='submit' value='  Створити ЧК ' onclick='validate1();'></td></tr></table>";
  ...
  echo "<input type='submit' value='  Створити ЧК ' onclick='validate2();'></td></tr></table>";
