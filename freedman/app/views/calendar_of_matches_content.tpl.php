...
<div class="calendar-of-matches__head-nav">
    ...
    <div class="swiper swiper-month-controls swiper-initialized swiper-horizontal">
        <div class="swiper-wrapper swiper-wrapper-month-controls" id="swiper-wrapper-2894150f39673565"
            ...
            <?php foreach($dateMatches as $dateMatch): ?>
            <div class="swiper-slide swiper-slide-month-controls swiper-slide-active" role="group"
                ...
                <a 
                    ...
                    data-turid="<?= $dateMatch['tur'] ?>"
                    data-selecteddate = "<?= $dateMatch['date'] ?>"

                    class="month-controls__button 
                    <?= $dateMatch['canseled'] == "1" ? 'month-controls__button--past' : '' ?> 
                    <?= $dateMatch['date'] == $selectedDate ? 'month-controls__button--current' : '' ?>"

                    <?= $dateMatch['date'] !== $selectedDate ? "href='{$site_url}/{$tournament}?tur={$dateMatch['tur']}&date={$dateMatch['date']}'" : ''?>
                >
                    <p><?= $dateMatch['month'] ?></p>
                    <p><?= $dateMatch['day']?></p>
                </a>
                ...
                <div class="card-of-matches">
                    ...
                    <div class="card-of-matches__controls">
                        ...
                        <a 
                            ...
                            title="Анонс матчу"
                            id="anons-<?= $i ?>"
                            <?= ($currentTur <= $lastTur && $dateLastTur >= $dateNow && $i == 1) ? 'style="background:red"': '' ?>
                        >  
...
<div class="calendar-of-matches__dynamic-content">
        <section class="green-zone content-to-capture">
            <?php if($currentTur <= $lastTur && $dateLastTur <= $dateNow) : ?>
